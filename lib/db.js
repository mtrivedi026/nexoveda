...
// --- AES-256 Encryption Setup for Patient Data Privacy ---
const ENCRYPTION_KEY = process.env.ENCRYPTION_KEY || 'nexoveda_super_secret_aes_key_32b'; 
...
const UserSchema = new mongoose.Schema({
  ...
  specialty: { type: String, enum: ['herbal', 'medical', 'mental_health', null], default: null },
...
const ConversationSchema = new mongoose.Schema({
  ...
  preferredSpecialty: { type: String, enum: ['herbal', 'medical', 'mental_health'], required: true },
...
const OtpSchema = new mongoose.Schema({
  ...
  purpose: { type: String, enum: ['register', 'forgot_password'], required: true },
  createdAt: { type: Date, default: Date.now, expires: 600 } // 10 minutes expiry
});

let MongoUser, MongoProduct, MongoConversation, MongoMessage, MongoOrder, MongoOtp;
...
try {
  ...
  MongoOtp = mongoose.models.OtpToken || mongoose.model('OtpToken', OtpSchema);
} catch (e) {}
...
class MockModel {
  ...
  saveCollection(items) {
    ...
    // Encrypt messages for mock JSON DB as well to ensure security across both DBs
    if (this.collectionName === 'messages') {
      const encryptedItems = items.map(item => ({
        ...item,
        text: encryptText(item.text)
      }));
      db[this.collectionName] = encryptedItems;
    } else {
  ...
  _populate(item) {
    ...
    if (this.collectionName === 'messages' && cloned.text) {
      cloned.text = decryptText(cloned.text);
    }
  ...
  async create(data) {
    ...
    if (this.collectionName === 'conversations' && !refNum) {
      refNum = 'NEXO-' + Math.random().toString(36).substring(2, 7).toUpperCase();
    }
    const newItem = {
      _id: generateId(),
      ...data,
...
async function syncAgents(dbInstance) {
  try {
    const hashedPassword = '$2b$10$3QKDQqAxO0NRNhu5UHKxLObFQfFMnEQlq/ZEyyl9DWm8mKhO/rsBm'; // password123
    
    // 1. Delete all agents that are not Anamika, Harsh, or Smita
    await dbInstance.User.deleteMany({
      role: 'agent',
      email: { $nin: ['anamika@nexoveda.com', 'harsh@nexoveda.com', 'smita@nexoveda.com'] }
    });

    // 2. Delete all other admins that are not admin@nexoveda.com
    await dbInstance.User.deleteMany({
      role: 'admin',
      email: { $nin: ['admin@nexoveda.com'] }
    });

    // 3. Delete mock test customer john@example.com and deprecated agent Anil
    await dbInstance.User.deleteMany({
      email: { $in: ['john@example.com', 'anil@nexoveda.com'] }
    });
    
    const anamikaData = {
      name: 'Female Herbal Consultant',
    ...
    const smitaData = {
      name: 'Female Mental Health Support Specialist',
    ...
    // 5. Force Upsert Dr. Harsh Rawat
    const harshExists = await dbInstance.User.findOne({ email: 'harsh@nexoveda.com' });
    if (harshExists) {
      await dbInstance.User.findByIdAndUpdate(harshExists._id, harshData);
      console.log('🔄 Forced update of Dr. Harsh Rawat credentials.');
    } else {
      await dbInstance.User.create({ _id: 'agent-harsh', ...harshData });
      console.log('🌱 Seeded Dr. Harsh Rawat (B.A.M.S.) agent successfully.');
    }

    // 6. Force Upsert Ms. Smita Gupta
    const smitaExists = await dbInstance.User.findOne({ email: 'smita@nexoveda.com' });
    if (smitaExists) {
      await dbInstance.User.findByIdAndUpdate(smitaExists._id, smitaData);
      console.log('🔄 Forced update of Ms. Smita Gupta credentials.');
    } else {
      await dbInstance.User.create({ _id: 'agent-smita', ...smitaData });
      console.log('🌱 Seeded Ms. Smita Gupta (M. A. Psychology) agent successfully.');
    }

    // 7. Force Upsert Nexoveda Admin
...
const MockOtp = new MockModel('otptokens');
...
const OtpProxy = createModelProxy(() => MongoOtp, () => MockOtp);
