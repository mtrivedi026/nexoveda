...
let MongoUser, MongoProduct, MongoConversation, MongoMessage, MongoOrder, MongoSpecialist, MongoQuizQuestion;
...
try {
  ...
  MongoSpecialist = mongoose.models.Specialist || mongoose.model('Specialist', SpecialistSchema);
  MongoQuizQuestion = mongoose.models.QuizQuestion || mongoose.model('QuizQuestion', QuizQuestionSchema);
} catch (e) {}
...
const MockSpecialist = new MockModel('specialists');
const MockQuizQuestion = new MockModel('quizquestions');

async function syncAgents(dbInstance) {
  try {
    const hashedPassword = '$2b$10$gJAo3x5pVsHsJV7WZd4lr.kjq7Pd/xv8vxmjYNPzjig3rH/gEoY8e';
    
    // 1. Delete all agents that are not Anamika or Anil
    await dbInstance.User.deleteMany({
      role: 'agent',
      email: { $nin: ['anamika@nexoveda.com', 'anil@nexoveda.com'] }
    });

    // 2. Delete all other admins that are not admin@nexoveda.com
    await dbInstance.User.deleteMany({
      role: 'admin',
      email: { $ne: 'admin@nexoveda.com' }
    });

    // 3. Delete mock test customer john@example.com
    await dbInstance.User.deleteMany({
    ...
    const anilData = {
      name: 'Dr. Anil Singh (B.A.M.S.)',
      email: 'anil@nexoveda.com',
      role: 'agent',
      specialty: 'herbal',
      gender: 'male',
      password: hashedPassword,
    ...
    // 4. Force Upsert Dr. Anamika Verma
    const anamikaExists = await dbInstance.User.findOne({ email: 'anamika@nexoveda.com' });
    if (anamikaExists) {
      await dbInstance.User.findByIdAndUpdate(anamikaExists._id, anamikaData);
      console.log('🔄 Forced update of Dr. Anamika Verma credentials.');
    } else {
      await dbInstance.User.create({ _id: 'agent-anamika', ...anamikaData });
      console.log('🌱 Seeded Dr. Anamika Verma (Gyne) agent successfully.');
    }
    
    // 5. Force Upsert Dr. Anil Singh
    const anilExists = await dbInstance.User.findOne({ email: 'anil@nexoveda.com' });
    if (anilExists) {
      await dbInstance.User.findByIdAndUpdate(anilExists._id, anilData);
      console.log('🔄 Forced update of Dr. Anil Singh credentials.');
    } else {
      await dbInstance.User.create({ _id: 'agent-anil', ...anilData });
      console.log('🌱 Seeded Dr. Anil Singh (B.A.M.S.) agent successfully.');
    }

    // 6. Force Upsert Nexoveda Admin
    const adminExists = await dbInstance.User.findOne({ email: 'admin@nexoveda.com' });
    if (adminExists) {
      await dbInstance.User.findByIdAndUpdate(adminExists._id, adminData);
      console.log('🔄 Forced update of Nexoveda Admin credentials.');
    } else {
      await dbInstance.User.create({ _id: 'admin-main', ...adminData });
      console.log('🌱 Seeded Nexoveda Admin successfully.');
    }
  } catch (err) {
    console.warn('⚠️ Force agent/user synchronization failed:', err.message);
  }
}

async function syncSpecialists(dbInstance) {
  try {
    const defaultSpecs = [
      { name: 'Male Ayurvedic Doctor', timing: '12:00 PM to 3:00 PM', active: true },
      { name: 'Female Doctor', timing: '6:00 PM to 8:00 PM', active: true },
      { name: 'Female Mental Support Specialist', timing: '8:00 PM to 9:00 PM', active: true }
    ];

    const count = await dbInstance.Specialist.find({});
    if (count.length === 0) {
      for (const spec of defaultSpecs) {
        await dbInstance.Specialist.create(spec);
      }
      console.log('🌱 Seeded default specialist timings.');
    }
  } catch (err) {
    console.warn('⚠️ Specialist sync failed:', err.message);
  }
}

async function syncQuizQuestions(dbInstance) {
  try {
    const defaultQuestions = [
      {
        text: 'How often do you experience physical fatigue during the day?',
        options: [
          { text: 'Rarely', score: 1 },
      ...
      {
        text: 'How would you rate your muscle recovery after physical activities?',
        options: [
          { text: 'Excellent', score: 1 },
          { text: 'Moderate', score: 2 },
          { text: 'Very Slow', score: 3 }
        ],
        active: true
      },
      {
        text: 'How would you rate your daily focus and mental clarity?',
        options: [
          { text: 'Sharp', score: 1 },
          { text: 'Average', score: 2 },
          { text: 'Easily distracted / Foggy', score: 3 }
        ],
        active: true
      }
    ];

    const count = await dbInstance.QuizQuestion.find({});
    if (count.length === 0) {
      for (const q of defaultQuestions) {
        await dbInstance.QuizQuestion.create(q);
      }
      console.log('🌱 Seeded default health quiz questions.');
    }
  } catch (err) {
    console.warn('⚠️ Quiz question sync failed:', err.message);
  }
}

let isConnectedCached = false;
let connectionPromise = null;

async function connectDB() {
  if (!process.env.MONGODB_URI) {
    isMock = true;
    if (isConnectedCached) return;
    console.log('⚠️ MONGODB_URI not configured. Using Local JSON Mock Database.');
    // Mark connected FIRST so failures in seed/sync don't block future requests
    isConnectedCached = true;
    try {
      const dbInstance = module.exports;
      const products = await dbInstance.Product.find({});
      if (products.length === 0) {
        console.log('📭 Mock Database appears to be empty. Running auto-seed...');
        const { seedDatabase } = require('./seed');
        await seedDatabase(dbInstance);
      }
      await syncAgents(dbInstance);
      await syncSpecialists(dbInstance);
      await syncQuizQuestions(dbInstance);
    } catch (mockSeedErr) {
      console.warn('⚠️ Mock Auto-seeding check failed:', mockSeedErr.message);
    }
    return;
  }
  ...
  connectionPromise = (async () => {
    try {
      // Set bufferTimeoutMS globally to 30s
      ...
      // Run seed/sync in background - don't block login
      (async () => {
        try {
          const dbInstance = module.exports;
          const products = await dbInstance.Product.find({});
          if (products.length === 0) {
            console.log('📭 MongoDB Database appears to be empty. Running auto-seed...');
            const { seedDatabase } = require('./seed');
            await seedDatabase(dbInstance);
          }
          await syncAgents(dbInstance);
      ...
      await syncQuizQuestions(dbInstance);
          ...
          console.error('⚠️ Auto-seeding check failed:', seedErr.message);
        }
      })();
    } catch (err) {
      isMock = true;
      isConnectedCached = true; // Mark as done so we stop retrying on every request
...
const SpecialistProxy = createModelProxy(() => MongoSpecialist, () => MockSpecialist);
const QuizQuestionProxy = createModelProxy(() => MongoQuizQuestion, () => MockQuizQuestion);
...
module.exports = {
  ...
  Order: OrderProxy,
  Specialist: SpecialistProxy,
  QuizQuestion: QuizQuestionProxy
};
