...
async function sendConsultationNotification(conv) {
  ...
  if (conv.agent) {
    if (typeof conv.agent === 'object') {
      ...
      agentName = conv.agent.toString().includes('harsh') || conv.agent === '605c72ab1c2d3a0015f623a1'
        ? 'Male Herbal Consultant'
        : 'Female Herbal Consultant';
    }
  } else {
    // Determine based on preferredGender
    if (conv.preferredGender === 'male') {
      agentName = 'Male Herbal Consultant';
    } else if (conv.preferredGender === 'female') {
      agentName = 'Female Herbal Consultant';
    }
  }
...
Access the staff portal at: https://nexoveda.com/staff`;
  ...
  try {
    ...
    // Force use of FormSubmit.co for consultation alerts to bypass Hostinger SMTP port blocking
    if (false) {
      ...
      console.log('⚠️ Bypassing nodemailer and using FormSubmit.co to avoid Hostinger SMTP block.');
