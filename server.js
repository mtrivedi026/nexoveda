...
async function startServer() {
  ...
  // --- SPECIALISTS TIMINGS MANAGEMENT API ---
  ...
  app.post('/api/specialists', authenticateToken, async (req, res) => {
    try {
      ...
      const { Specialist } = db;
      const { _id, name, timing, active } = req.body;
      if (!name || !timing) return res.status(400).json({ message: 'Missing name or timing.' });
  ...
  app.delete('/api/specialists/:id', authenticateToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).json({ message: 'Unauthorized.' });
      const { Specialist } = db;
      await Specialist.deleteMany({ _id: req.params.id });
      res.json({ success: true, message: 'Specialist timing deleted.' });
    } catch (err) {
      res.status(500).json({ message: err.message });
    }
  });

  // --- HEALTH QUIZ QUESTIONS CRUD & ASK API ---
  app.get('/api/quiz/questions', async (req, res) => {
    try {
      const { QuizQuestion } = db;
  ...
  app.post('/api/quiz/questions', authenticateToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).json({ message: 'Unauthorized.' });
      const { QuizQuestion } = db;
      const { _id, text, options, active } = req.body;
      if (!text || !options || !Array.isArray(options)) return res.status(400).json({ message: 'Missing required parameters.' });
  ...
  app.delete('/api/quiz/questions/:id', authenticateToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).json({ message: 'Unauthorized.' });
      const { QuizQuestion } = db;
      await QuizQuestion.deleteMany({ _id: req.params.id });
  ...
  app.post('/api/quiz/ask', async (req, res) => {
    try {
      ...
      res.status(500).json({ answer: 'Sorry, failed to process query.' });
    }
  });
