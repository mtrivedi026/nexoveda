...
export default function AnamikaLoginPage() {
  ...
  // Redirect if already logged in as Anamika
  useEffect(() => {
    if (user && token) {
      if (user.role === 'agent' && user.email === 'anamika@nexoveda.com') {
        window.location.href = '/agent/anamika';
      } else {
        setError('Your current session is not Dr. Anamika Verma. Please log in with her advisor credentials.');
      }
    }
  }, [user, token]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (targetEmail !== 'anamika@nexoveda.com') {
      setError('This portal is restricted exclusively for Dr. Anamika Verma.');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (data.user.role !== 'agent' || data.user.email !== 'anamika@nexoveda.com') {
        throw new Error('Authenticated user is not Dr. Anamika Verma.');
      }
  ...
  return (
    <div className="min-h-screen bg-[#030906] text-gray-100 flex flex-col justify-between relative overflow-hidden">
        ...
        <div className="w-full max-w-md bg-emerald-950/10 border border-emerald-900/20 rounded-3xl p-8 backdrop-blur-xl shadow-2xl relative overflow-hidden group">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-xs font-bold uppercase tracking-wider text-yellow-400 mt-2">
              Female Intimate Wellness Desk Portal
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <input
                ...
                placeholder="anamika@nexoveda.com"
            ...
            >
              {loading ? 'Verifying Credentials...' : 'Sign In as Dr. Anamika'}
            </button>
          </form>
      ...
      <footer className="py-6 border-t border-emerald-950/40 text-center text-xs text-gray-600">
        © 2026 Nexoveda Global. Advisor Desk. Secure Server Connection.
      </footer>
    </div>
  );
}
