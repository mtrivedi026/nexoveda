...
export default function AnilLoginPage() {
  ...
  // Redirect if already logged in as Anil
  useEffect(() => {
    if (user && token) {
      if (user.role === 'agent' && user.email === 'anil@nexoveda.com') {
        window.location.href = '/agent/anil';
      } else {
        setError('Your current session is not Dr. Anil Singh. Please log in with his advisor credentials.');
      }
    }
  }, [user, token]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (targetEmail !== 'anil@nexoveda.com') {
      setError('This portal is restricted exclusively for Dr. Anil Singh.');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (data.user.role !== 'agent' || data.user.email !== 'anil@nexoveda.com') {
        throw new Error('Authenticated user is not Dr. Anil Singh.');
      }
        ...
        <div className="w-full max-w-md bg-emerald-950/10 border border-emerald-900/20 rounded-3xl p-8 backdrop-blur-xl shadow-2xl relative overflow-hidden group">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-xs font-bold uppercase tracking-wider text-yellow-400 mt-2">
              Male Ayurvedic Specialist Portal
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              {loading ? 'Verifying Credentials...' : 'Sign In as Dr. Anil'}
            </button>
          </form>
