...
export default function HarshAgentPage() {
  ...
  useEffect(() => {
    if (!authLoading) {
      if (!user || user.role !== 'agent' || user.email !== 'harsh@nexoveda.com') {
  ...
  useEffect(() => {
    if (!token || !user || user.role !== 'agent' || user.email !== 'harsh@nexoveda.com') return;
  ...
  if (authLoading || !user || user.role !== 'agent' || user.email !== 'harsh@nexoveda.com') {
    return (
  ...
  return (
    <div className="min-h-screen bg-white text-slate-800 flex flex-col">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-grow w-full flex flex-col gap-6">
        ...
        <section className="bg-slate-50 border border-slate-200 rounded-3xl p-6 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            />
            <div>
              <h2 className="text-xl font-bold text-slate-800">Male Herbal Consultant</h2>
              <p className="text-xs text-slate-500 font-medium capitalize">
                Role: Male Herbal Consultation Specialist ({user.gender})
              </p>
            </div>
          </div>
      ...
      <footer className="bg-slate-50 border-t border-slate-200 py-8 text-center text-xs text-slate-500 mt-12">
        © 2026 Nexoveda Global. Male Herbal Consultant Desk. All rights reserved.
      </footer>
    </div>
  );
}
