...
export default function AnilAgentPage() {
  ...
  useEffect(() => {
    if (!authLoading) {
      if (!user || user.role !== 'agent' || user.email !== 'anil@nexoveda.com') {
        window.location.href = '/login/anil';
      } else {
  ...
  useEffect(() => {
    if (!token || !user || user.role !== 'agent' || user.email !== 'anil@nexoveda.com') return;
  ...
  if (authLoading || !user || user.role !== 'agent' || user.email !== 'anil@nexoveda.com') {
    return (
  ...
  return (
    <div className="min-h-screen bg-[#050e0a] text-gray-100 flex flex-col">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-grow w-full flex flex-col gap-6">
        ...
        <section className="bg-emerald-950/15 border border-emerald-900/30 rounded-3xl p-6 flex flex-col sm:flex-row justify-between items-center gap-4 backdrop-blur-md">
          <div className="flex items-center gap-3">
            <img
              src={user.avatarUrl || 'https://images.unsplash.com/photo-1537368910025-700350fe46c7?w=150&auto=format&fit=crop&q=80'}
              alt={user.name}
              className="w-14 h-14 rounded-full object-cover border border-emerald-800"
            />
            <div>
              <h2 className="text-xl font-bold text-white">Dr. Anil Singh (B.A.M.S.)</h2>
              <p className="text-xs text-emerald-400 font-medium capitalize">
                Role: Male Ayurvedic Specialist ({user.gender})
              </p>
            </div>
          </div>
      ...
      <footer className="bg-emerald-950/20 border-t border-emerald-900/30 py-8 text-center text-xs text-emerald-600 mt-12">
        © 2026 Nexoveda Global. Dr. Anil Singh Desk. All rights reserved.
      </footer>
    </div>
  );
}
